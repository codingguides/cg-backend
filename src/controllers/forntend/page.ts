...
FrontendController.get(
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      await TopicModel.find({ showNav: true })
        .sort({ index_no: 1 })
        .then((val) => {
          if (val) {
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      await TopicModel.findOne({ slug: slug }).then(async (val) => {
        ...
        if (val && val._id) {
          console.log("if");
          await TopicModel.find({ parent_id: ObjectId(val._id) }).then(
            (sublist) => {
              if (sublist) {
      ...
      await TopicModel.findOne({ slug: slug }).then(async (val) => {
        ...
        if (val && val._id) {
          await RelationModel.find(
            { topic_id: ObjectId(val._id) },
            { question_id: 1 }
          ).then(async (relationdata) => {
            const relationIds = relationdata.map((rel) =>
              ObjectId(rel.question_id)
            );
            console.log(
              relationIds,
              "<<<<<<<<<<<<relationdata>>>>>>>>>>>>>",
            ...
            if (relationdata) {
              await QuestionModel.find({ _id: { $in: relationIds } }).then(
                async (questions) => {
...
FrontendController.get(
  ...
  async (request: Request, response: Response, next: NextFunction) => {
    try {
      ...
      await BlogModel.findOne({ sort_slug: slug }).then(async (val) => {
        if (val) {
          let relatedBlogList = await BlogModel.find({
